                    ...
                    <div class="card course-card h-100 border-0 shadow-sm">
                        <div class="card-body">
                            ...
                            <div class="course-info">
                                ...
                                <div class="mb-2" data-time="{{ $course->start_time }}|{{ $course->end_time }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Course selection functionality with conflict checking
        function updateSubmitButton() {
            const checkedBoxes = document.querySelectorAll('.course-checkbox:checked');
            const conflictWarning = document.getElementById('conflictWarning');

            // Check for time conflicts
            const conflicts = checkTimeConflicts(checkedBoxes);
                ...
                if (!conflictWarning) {
                    const warningHtml = `
                            <div id="conflictWarning" class="alert alert-warning mt-3" style="display: none;">
                                ...
                                <strong>تحذير: يوجد تعارض في المواعيد</strong>
                                <div id="conflictDetails" class="mt-2 small"></div>
                            </div>
                        `;
                    document.querySelector('#coursesGrid').insertAdjacentHTML('afterend', warningHtml);
                }

                document.getElementById('conflictWarning').style.display = 'block';
                document.getElementById('conflictDetails').innerHTML = conflicts.join('<br>');

                // Disable submit button
                submitBtn.disabled = true;
                submitBtn.classList.add('btn-secondary');
                submitBtn.classList.remove('btn-primary');
                submitHint.innerHTML = `<i class="bx bx-error-circle me-1 text-warning"></i>يرجى حل تعارض المواعيد قبل المتابعة`;
        ...
        function checkTimeConflicts(checkedBoxes) {
            const conflicts = [];
            const courses = [];

            // Collect course data for checked courses
            checkedBoxes.forEach(checkbox => {
                const courseCard = checkbox.closest('.course-card');
                const courseId = checkbox.value;
                const courseName = checkbox.closest('.course-card').querySelector('.form-check-label').textContent.trim();
                const timeElement = courseCard.querySelector('.course-info [data-time]');
                ...
                if (timeElement) {
                    ...
                    if (timeData) {
                        try {
                            ...
                            console.warn('Invalid time data for course:', courseName);
                        }
                    }
                }
            });

            // Check for conflicts between each pair of courses
            for (let i = 0; i < courses.length; i++) {
                for (let j = i + 1; j < courses.length; j++) {
                    ...
                    if (timesOverlap(course1.startTime, course1.endTime, course2.startTime, course2.endTime)) {
                        const conflict = `دورة "${course1.name}" و "${course2.name}" لهما نفس التوقيت: ${formatDateTime(course1.startTime)} - ${formatDateTime(course1.endTime)}`;
