        ...
        <div class="row justify-content-center">
            <div class="col-lg-8">
                <div class="success-container">
                    ...
                    <p class="lead mb-4">شكرًا لك على التقديم في دوراتنا التدريبية</p>

                    <!-- Student Code -->
                    <div class="student-code">
                        <h4 class="mb-2">رقم الطلب الخاص بك</h4>
                        <div class="code-text">{{ session('application_code') }}</div>
                    ...
                    <div class="info-card">
                        ...
                        <div class="row text-start">
                            <div class="col-md-6 mb-2">
                                <strong>الاسم:</strong> {{ session('student_name', 'غير محدد') }}
                            </div>
                            <div class="col-md-6 mb-2">
                                <strong>البريد الإلكتروني:</strong> {{ session('student_email', 'غير محدد') }}
                            </div>
                            <div class="col-md-6 mb-2">
                                <strong>الهاتف:</strong> {{ session('student_phone', 'غير محدد') }}
                            </div>
                            <div class="col-md-6 mb-2">
                                <strong>التخصص:</strong> {{ session('category_name', 'غير محدد') }}
                            </div>
                            <div class="col-12 mb-2">
                                <strong>حالة الطلب:</strong>
                                <span
                                    class="badge bg-warning text-dark">{{ session('status_message', 'في الانتظار') }}</span>
                            </div>
                        </div>
                    </div>

                    <!-- Selected Courses -->
                    @if (session('selected_courses'))
                        <div class="info-card">
                            <h5 class="mb-3">
                                <i class="bx bx-book-open me-2"></i>
                                الدورات المختارة ({{ count(session('selected_courses', [])) }} دورة)
                            </h5>
                            <div class="row">
                                @foreach (session('selected_courses', []) as $course)
                                    <div class="col-md-6 mb-2">
                                        <div class="d-flex align-items-center">
                                            <i class="bx bx-check-circle text-success me-2"></i>
                                            <div>
                                                <div class="fw-bold">{{ $course['title'] ?? 'غير محدد' }}</div>
                                                <small class="text-light">{{ $course['category'] ?? 'غير محدد' }}</small>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    <!-- Action Buttons -->
                    <div class="mt-4">
                        <a href="{{ route('applications.form') }}" class="btn btn-home me-3">
